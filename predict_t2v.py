...
if config['text_encoder_kwargs'].get('enable_multi_text_encoder', False):
    text_encoder = BertModel.from_pretrained(model_name, subfolder="text_encoder").to(torch.bfloat16)
    text_encoder_2 = T5EncoderModel.from_pretrained(model_name, subfolder="text_encoder_2").to(torch.bfloat16)
else:
...
if transformer.config.in_channels != vae.config.latent_channels and config['transformer_additional_kwargs'].get('enable_clip_in_inpaint', True):
    clip_image_encoder = CLIPVisionModelWithProjection.from_pretrained(model_name, subfolder="image_encoder").to(weight_dtype)
...
if config['text_encoder_kwargs'].get('enable_multi_text_encoder', False):
    ...
    if transformer.config.in_channels != vae.config.latent_channels:
        ...
        pipeline = EasyAnimatePipeline.from_pretrained(
            model_name, text_encoder=text_encoder, tokenizer=tokenizer, vae=vae, transformer=transformer, scheduler=scheduler, torch_dtype=weight_dtype
        )
